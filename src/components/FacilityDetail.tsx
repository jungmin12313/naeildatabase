        ...
        {scores.map(scoreObj => {
          ...
          catMeasurements.forEach(m => {
             ...
             if (typeof m.value === 'boolean') {
               formattedVal = m.value ? '있음 (O)' : '없음 (X)';
             } else if (m.value !== null && m.unit && !String(m.value).includes(m.unit)) {
               formattedVal = `${m.value}${m.unit}`;
             }
             // 값이 존재하면 덮어쓰거나 새로 추가
             if (formattedVal !== null && formattedVal !== undefined && formattedVal !== '') {
               rawMetrics[m.field_name] = formattedVal;
             }
          });
          ...
          return (
            <section key={scoreObj.id} className="bg-white rounded-2xl border border-zinc-200 overflow-hidden shadow-sm print:break-inside-avoid">
              ...
              <div className="p-4 space-y-4">
                ...
                {/* 접근성 달성 및 개선 가이드 (모든 영역 표시) */}
                <details className={`group mt-4 border rounded-xl overflow-hidden ${scoreObj.score !== null && scoreObj.score >= 100 ? 'border-blue-200 bg-blue-50/30' : 'border-red-200 bg-red-50/30'}`}>
                  <summary className={`cursor-pointer p-3 flex justify-between items-center transition-colors list-none outline-none ${scoreObj.score !== null && scoreObj.score >= 100 ? 'bg-blue-50 hover:bg-blue-100' : 'bg-red-50 hover:bg-red-100'}`}>
                    <span className={`text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 ${scoreObj.score !== null && scoreObj.score >= 100 ? 'text-blue-700' : 'text-red-600'}`}>
                      <AlertTriangle size={14} /> {scoreObj.score !== null && scoreObj.score >= 100 ? '접근성 기준 달성 확인 (가이드)' : '미준수 항목 바로 개선 가이드'}
                    </span>
                    <span className={`group-open:rotate-180 transition-transform ${scoreObj.score !== null && scoreObj.score >= 100 ? 'text-blue-500' : 'text-red-400'}`}><ChevronRight size={14} /></span>
                  </summary>
                  <div className={`p-4 text-xs text-zinc-700 bg-white border-t space-y-3 ${scoreObj.score !== null && scoreObj.score >= 100 ? 'border-blue-100' : 'border-red-100'}`}>
                    <p className={`font-semibold ${scoreObj.score !== null && scoreObj.score >= 100 ? 'text-blue-800' : 'text-red-800'}`}>🛠️ 접근성 100점 필수 시공/개선 기준 가이드:</p>
